// ══════════════════════════════════════════════════════════════════════════════
// TALENTSIGNAL — Pure Vanilla JavaScript. No frameworks, no libraries.
// ══════════════════════════════════════════════════════════════════════════════
...
window.onload = function() {
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    o.value = i; o.textContent = i + " Sample Resume" + (i>1?"s":"");
  ...
  for (let i = 1; i <= 30; i++) {
    const o = document.createElement("option");
    o.value = i; o.textContent = i + " PDF Resume" + (i>1?"s":"");
...
function saveKey() {
  ...
  try { localStorage.setItem("ts_key", apiKey); } catch(e) {}
}
...
function onSampleCountChange() {
  sampleCount = parseInt(document.getElementById("sample-count-select").value);
  updateAnalyzeBtn();
}

function onPdfCountChange() {
  pdfCount = parseInt(document.getElementById("pdf-count-select").value);
  buildPdfSlots(pdfCount);
  updateAnalyzeBtn();
}
...
function renderPdfGrid() {
  ...
  pdfSlots.forEach(function(slot, i) {
    ...
    div.className = "pdf-slot" + (slot.file ? " filled" : "");
    if (slot.file) {
      div.innerHTML = `<div class="pdf-slot-num">RESUME #${i+1}</div>
        <div class="pdf-filled-name">✓ ${slot.fileName}</div>
        <input type="text" placeholder="Candidate name" value="${slot.name}" oninput="updateSlotName('${slot.id}',this.value)" style="font-size:11px;padding:5px 8px;margin-bottom:6px;"/>
        <button onclick="removePdf('${slot.id}')" class="btn-ghost" style="font-size:10px;padding:3px 8px;color:#FF4F4F;border-color:rgba(255,79,79,.25);width:100%;">✕ Remove</button>`;
    } else {
      div.innerHTML = `<div class="pdf-slot-num">RESUME #${i+1}</div>
        <label class="pdf-upload-label">
          <span style="font-size:24px;">📄</span>
          <span>Click to upload PDF</span>
          <input type="file" accept=".pdf" style="display:none;" onchange="handlePdfUpload('${slot.id}',this.files[0])"/>
        </label>`;
    }
...
async function handlePdfUpload(id, file) {
  ...
  const slot = pdfSlots.find(s=>s.id===id);
  if (!slot) return;
  slot.file     = file;
...
function resetToSetup() { results=[]; selectedId=null; activeFilter="ALL"; showPage("page-setup"); }
...
// ── RADAR CHART ───────────────────────────────────────────────────────────────
/*
  HOW THE RADAR CHART WORKS — explained simply:

  A radar chart is a circle divided into 4 axes (Skills, Experience, Education, Communication).
  Each axis goes from the centre (0) to the edge (100).
  We plot each score as a point on its axis, then connect all 4 points into a polygon shape.

  We draw it using SVG — a web standard for drawing shapes with code.

  The maths to find each point's x,y position uses basic trigonometry:
    - We split the circle into 4 equal slices: 360° / 4 = 90° apart
    - For each axis i, the angle is: (i/4) × 2π  (in radians, not degrees)
    - We subtract π/2 to start from the top instead of the right
    - x = centrex + cos(angle) × radius × (score/100)
    - y = centrey + sin(angle) × radius × (score/100)

  cos() and sin() are built-in JavaScript Math functions.
  They tell you how far to move horizontally (cos) and vertically (sin)
  when you travel along a circle at a given angle.

  That's it. No library needed — just a loop and basic maths.
*/
function radarSVG(scores, size) {
  const cx = size/2, cy = size/2, r = size*0.38;
  const dims = ["skills","experience","education","communication"];

  // Calculate the (x,y) point for each score on its axis
  const pts = dims.map(function(d, i) {
    var angle = (i/4) * Math.PI*2 - Math.PI/2; // equally spaced angles, start from top
    var value = (scores[d]||0) / 100;           // score as 0.0–1.0
    return [
      cx + Math.cos(angle) * r * value,         // x position
      cy + Math.sin(angle) * r * value          // y position
    ];
  });

  // Grid lines (faint background polygons at 25%, 50%, 75%, 100%)
  function gridRing(scale) {
    return dims.map(function(_, i) {
      var a = (i/4)*Math.PI*2 - Math.PI/2;
      return (cx+Math.cos(a)*r*scale)+","+(cy+Math.sin(a)*r*scale);
    }).join(" ");
  }

  // Axis lines from centre to edge
  var axisLines = dims.map(function(_, i) {
    var a = (i/4)*Math.PI*2 - Math.PI/2;
    return '<line x1="'+cx+'" y1="'+cy+'" x2="'+(cx+Math.cos(a)*r)+'" y2="'+(cy+Math.sin(a)*r)+'" stroke="#1E2D45" stroke-width=".5"/>';
  }).join("");

  var polyPts = pts.map(function(p){ return p.join(","); }).join(" ");

  return '<svg width="'+size+'" height="'+size+'" style="flex-shrink:0">'
    + '<polygon points="'+gridRing(.25)+'" fill="none" stroke="#1E2D45" stroke-width=".5"/>'
    + '<polygon points="'+gridRing(.5)+'"  fill="none" stroke="#1E2D45" stroke-width=".5"/>'
    + '<polygon points="'+gridRing(.75)+'" fill="none" stroke="#1E2D45" stroke-width=".5"/>'
    + '<polygon points="'+gridRing(1)+'"   fill="none" stroke="#1E2D45" stroke-width=".5"/>'
    + axisLines
    + '<polygon points="'+polyPts+'" fill="rgba(0,212,255,0.13)" stroke="#00D4FF" stroke-width="1.5" stroke-linejoin="round"/>'
    + '</svg>';
}
...
function renderStats() {
  document.getElementById("stats-row").innerHTML = [
    ...
    { label:"STRONG YES", val:results.filter(r=>r.recommendation==="STRONG_YES").length, color:"#00E676" },
    { label:"YES/MAYBE",  val:results.filter(r=>r.recommendation==="YES"||r.recommendation==="MAYBE").length, color:"#FFB74D" },
    { label:"BIAS FLAGS", val:results.filter(r=>r.biasFlags&&r.biasFlags.length>0).length, color:"#FF4F4F" },
...
function renderFilters() {
  document.getElementById("filter-row").innerHTML = ["ALL","STRONG_YES","YES","MAYBE","NO"].map(function(f) {
    const count = f==="ALL" ? "" : " ("+results.filter(r=>r.recommendation===f).length+")";
    return `<button class="filter-btn ${activeFilter===f?"active":""}" onclick="setFilter('${f}')">${f.replace("_"," ")}${count}</button>`;
  }).join("");
}
...
function getRanked() {
  // First sort ALL results by weighted score, assign global rank
  var sorted = results
    .map(function(c) { return { c:c, ws:weightedScore(c) }; })
    .sort(function(a,b) { return b.ws - a.ws; });

  // Assign rank based on full sorted order, then filter
  var withRank = sorted.map(function(item, i) {
    return { candidate: item.c, ws: item.ws, rank: i+1 };
  });

  // Apply filter
  return withRank.filter(function(item) {
    return activeFilter==="ALL" || item.candidate.recommendation===activeFilter;
  });
}
...
function renderList() {
  ...
  if (ranked.length===0) {
    document.getElementById("candidate-list").innerHTML='<div style="text-align:center;padding:40px;color:#6B8CAE;">No candidates match this filter.</div>';
    return;
  }
  document.getElementById("candidate-list").innerHTML = ranked.map(function(item, i) {
    var c     = item.candidate;
    var score = item.ws;
    var rank  = item.rank;
    var id    = c._id;
    var rl    = rankLabel(rank);
    var skillTags = (c.skills||[]).slice(0,4).map(function(s){ return '<span class="tag tag-dim">'+s+'</span>'; }).join("");
    var bias  = (c.biasFlags&&c.biasFlags.length>0) ? '<span class="tag tag-amber">⚠ bias</span>' : "";
    var isSelected = (String(selectedId) === String(id));
    return '<div class="card cand-card '+(isSelected?"selected":"")+'" onclick="selectCandidate(\''+id+'\')" style="animation-delay:'+( i*0.06)+'s">'
      + '<div class="cand-rank" style="font-size:'+(rank<=3?18:12)+'px;color:'+rl.col+';">'+rl.lbl+'</div>'

      + '<div class="cand-info">'
        + '<div class="cand-name-row"><span class="cand-name">'+c.name+'</span>'+recBadge(c.recommendation)+bias+'</div>'
        + '<div class="cand-sub">'+(c.headline||'')+' · '+(c.yearsExp||0)+'yr exp</div>'
        + '<div class="cand-skills">'+skillTags+'</div>'
      + '</div>'
      + '<div class="cand-score">'
        + '<div class="score-num" style="color:'+scoreColor(score)+';text-shadow:0 0 16px '+scoreColor(score)+'44">'+score+'</div>'
...
function onWeightChange() {
  weights.skills        = parseInt(document.getElementById("w-skills").value);
  weights.experience    = parseInt(document.getElementById("w-experience").value);
  weights.education     = parseInt(document.getElementById("w-education").value);
  weights.communication = parseInt(document.getElementById("w-communication").value);
  document.getElementById("w-skills-val").textContent        = weights.skills+"%";
  document.getElementById("w-experience-val").textContent    = weights.experience+"%";
  document.getElementById("w-education-val").textContent     = weights.education+"%";
  document.getElementById("w-communication-val").textContent = weights.communication+"%";
  renderList();
...
function barHTML(label, value) {
  ...
  return `<div class="bar-row">
    <div class="bar-header"><span class="bar-label">${label}</span><span class="bar-value" style="color:${color}">${value}</span></div>
    <div class="bar-bg"><div class="bar-fill" style="width:${value}%;background:${color};"></div></div>
  </div>`;
}

function renderDetail(id) {
  const c = results.find(r=>r._id==id);
  if (!c) return;
  const score = weightedScore(c);
  const bias = c.biasFlags&&c.biasFlags.length>0 ? `<div class="card" style="padding:14px;border-color:rgba(255,183,77,.3);margin-bottom:14px;">
    <div class="mono" style="font-size:11px;color:#FFB74D;margin-bottom:8px;letter-spacing:1px;">⚠ BIAS AUDIT FLAGS</div>
    ${c.biasFlags.map(f=>`<div style="font-size:12px;color:#FFB74D;margin-bottom:4px;">• ${f}</div>`).join("")}
  </div>` : "";
  ...
  document.getElementById("results-right").innerHTML = `
    <div style="animation:fadeUp .3s ease;">
      <div class="detail-header">
        ...
        <div style="text-align:right;"><div class="detail-score-big" style="color:${scoreColor(score)};text-shadow:0 0 26px ${scoreColor(score)}44">${score}</div>${recBadge(c.recommendation)}</div>
      </div>

      <div class="card" style="padding:16px;margin-bottom:14px;">
        <div class="mono" style="font-size:11px;color:#6B8CAE;margin-bottom:12px;letter-spacing:1px;">DIMENSION SCORES</div>
        <div>
            ${barHTML("SKILLS", c.scores.skills)}
            ${barHTML("EXPERIENCE", c.scores.experience)}
            ${barHTML("EDUCATION", c.scores.education)}
            ${barHTML("COMMUNICATION", c.scores.communication)}
          </div>
      </div>
      ...
      <div class="strengths-gaps">
        <div class="card" style="padding:14px;">
          ...
          ${(c.strengths||[]).map(s=>`<div class="list-item" style="border-left:2px solid #00E676;">${s}</div>`).join("")}
        </div>
        <div class="card" style="padding:14px;">
          <div class="mono" style="font-size:11px;color:#FF4F4F;margin-bottom:8px;letter-spacing:1px;">✗ GAPS</div>
          ${(c.gaps||[]).map(g=>`<div class="list-item" style="border-left:2px solid #FF4F4F;">${g}</div>`).join("")}
        </div>
      </div>
...
async function openEmail(id, type) {
  ...
  const prompt = type==="shortlist"
    ? `Write a warm professional shortlist email to ${c.name} for a Senior PM - AI/ML Products role. Highlight: ${(c.strengths||[]).slice(0,2).join(" and ")}. Mention next step is interview scheduling. Under 150 words. No subject line.`
    : `Write a respectful brief rejection email to ${c.name}. One genuine compliment. Under 100 words. No subject line.`;
  try {
    const text = await callAI([{role:"system",content:"You are a professional recruiter. Write concise warm emails."},{role:"user",content:prompt}], 400);
    document.getElementById("modal-text").value = text;
  } catch(e) { document.getElementById("modal-text").value = "Error: "+e.message; }
